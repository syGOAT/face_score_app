...
class DecoderFc(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        self.fc = nn.Linear(input_size, 196)
...
if __name__ == '__main__':
    net = PFLDInference()
    net.load_state_dict(torch.load('checkpoint/snapshot/checkpoint.pth.tar')['pfld_backbone'])
    encoder = PFLDEncoder()
    encoder.load_state_dict(torch.load('checkpoint/encoder.pth'))
    decoder = DecoderFc(176)
    decoder.load_state_dict(torch.load('checkpoint/fc.pth'))

    x = torch.arange(3*112*112).reshape(1, 3, 112, 112).type(torch.float32)
    _, y0 = net(x)
    x1 = encoder(x)
    y1 = decoder(x1)
